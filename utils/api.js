...
const STORAGE_DB_KEY = 'UdaciFlashcards:decks';
...
export function getDecks() {
  return AsyncStorage.getItem(STORAGE_DB_KEY).then(result => {
    if(result !== null) {
      return JSON.parse(result) 
    } else {
      AsyncStorage.setItem(STORAGE_DB_KEY, JSON.stringify(startingData));
      return startingData;
    }
  });
}
